...
def extract_tr_subject_data(path, subject, suffix, roi, conds):
    ...
    data = {'train_x': train_x_data, 'train_y': train_y_data,
            'test_x': test_x_data, 'test_y': test_y_data}
    return data
...
def get_optimal_run(x_train, y_train, x_test, y_test, kernels, gamma_range, C_range, n_cores):
    ...
    def evaluate_acc(params):
        svclassifier = SVC(
            kernel=params['kernel'], gamma=params['gamma'], C=params['C'], max_iter=-1)
...
def split_tr_dataset(data, scramble):
    ...
    test_indices, outer_test_indices, test_y, outer_test_y = train_test_split(
        test_indices, data['test_y'], test_size=2, stratify=data['test_y'])
    ...
    train_indices, inner_test_indices, _, inner_test_y = train_test_split(
        test_indices, test_y, test_size=6, stratify=test_y)
...
def train(data_params, grid_params, runs=200, scramble=False, n_cores=1):
    ...
    for subject in subjects:
        ...
        subject_data = extract_tr_subject_data(
            path, subject, suffix, roi, conds)
        x_train, x_test_inner, x_test_outer, y_train, y_test_inner, y_test_outer = split_tr_dataset(
            subject_data, scramble)
        ...
        for _ in tqdm(range(runs), leave=(not scramble)):
            ...
            svclassifier = SVC(
                kernel=opt_params['kernel'], gamma=opt_params['gamma'], C=opt_params['C'], max_iter=-1)
            svclassifier.fit(x_train, y_train)
...
parser.add_argument("-c", "--cond", action="store", type=int,
                    help="conditions: 0 is CP/Large, 1 is IP/Small")
